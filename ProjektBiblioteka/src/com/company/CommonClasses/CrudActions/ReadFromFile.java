...
//TODO:ODCZYT/ZAPIS , SERIALIZACJA?
public class ReadFromFile {//obsługa tabeli
  ...
  public ArrayList<graPlanszowa> read(String nazwaBazy) throws Exception { //ta metoda odczytuje całą tabelę z pliku i zwraca tablicę obiektów utworzonych z tej tabeli
    ...
    if (!new File(pulpit, nazwaBazy).canRead()) throw new FileNotFoundException(
      "nie udało się odczytać pliku z bazą"
    );
    ...
    while (bazaDanych.hasNextLine()) { //zapis kolejnych wierszy w tabeli do listy obiektów
      ...
      for (Field f : pola) {
            Class t = f.getType();
            if(!bazaDanych.hasNext())break;//edge case ,koniec pliku
            String temp = bazaDanych.next();
      ...
      if(bazaDanych.hasNextLine())bazaDanych.nextLine();//edge case w ostatnim wierszu tabeli
