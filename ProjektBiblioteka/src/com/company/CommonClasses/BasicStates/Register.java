...
public class Register {
    ...
    public Register() throws Exception {
        ...
        if(!issue && firstName.length()>0 && lastName.length()>0 && age>=14){
            UserList.add(new User(UserList.size()+1, firstName, lastName, "", age, false));

            WriteToFile zapis = new WriteToFile();
            zapis.write("users.csv", UserList);


            System.out.println("Pomyslnie utworzono uzytkonika --> " + new User(UserList.size(), firstName, lastName, "", age, false));
        }
        else {
